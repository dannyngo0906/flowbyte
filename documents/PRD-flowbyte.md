...
| **US-003** | F6: Data Transformation YAML | rename / skip / type_override field qua YAML config, validate trước save | S | US-001 | S1 | ✅ Done |
| **US-004** | F1: Core Sync Engine | ETL pipeline đầy đủ: 6 resources Phase 1, scheduler APScheduler, token bucket rate limit | L | US-001, US-002, US-003 | S1 | ✅ Done |
| **US-005** | F7: Alerting Telegram | Alert sync fail + scheduler dead, không spam, `flowbyte alert test` | S | US-004 | S2 | ✅ Done |
| **US-006** | F5: Data Validation | 2 rules: row count delta ≤10% + required fields not null, lưu `validation_results` | S | US-004 | S2 | ✅ Done |
| **US-007** | F3: CLI Observability | `status` / `history` / `logs` commands, 4 trạng thái, filter `--errors` / `--tail` / `--since` | S | US-004 | S2 | ✅ Done |
...
> **✅ Done (US-003):** Tất cả tiêu chí "Done khi" đạt — YAML parse (Pydantic model), rename/skip/type_override logic (unit tests + chạy qua code path thật trong US-004 E2E), `flowbyte validate` gọi `validate_transform_for_resource()` và báo lỗi (`pipeline.py:111-125`), edge cases skip `id` + rename collision + type_override fail đều implement và test. **Bug fix (2026-04-24 post-audit):** `runner.py:162` hardcode `None` cho variants transform config → bỏ qua config YAML của user cho `variants` resource → fixed bằng cách lookup `self._cfg.resources.get("variants")` và pass `variant_resource_cfg.transform`. 401/401 unit tests xanh sau fix.
> **✅ Done (US-004) — 2026-04-24:** Tất cả 5 resources (orders, customers, products, variants, locations) sync thành công E2E với Haravan API thật. Phát hiện thêm 2 bugs: (3) `inventory_levels` endpoint không tồn tại trong Haravan API — trả 200 + empty body; inventory data được embed trong `variants.inventory_advance` (JSONB) → resource disabled, schema variants cập nhật; (4) Full refresh mode vẫn pass checkpoint vào paginate → dedup skip toàn bộ records → fix: chỉ pass checkpoint khi mode == "incremental". Kết quả DB: orders=3, customers=3, products=4, variants=4 (với inventory_advance), locations=9. `inventory_levels` disabled trong pipeline YAML.
> **Known limitations (by design, không phải bug):** (a) Variants table không có `_deleted_at` — theo spec §⑦ variants là nested trong products, không có independent lifecycle; deleted variant sẽ không xuất hiện khi fetch products nhưng row cũ vẫn còn trong DB (acceptable cho MVP, clean-up qua Weekly Full Refresh products); (b) `inventory_levels` disabled trong YAML — nếu user enable trong YAML sẽ chạy nhưng trả empty data vì Haravan không có endpoint; không có warning trong config validate.
> **✅ Done (US-005) — 2026-04-24:** Tất cả tiêu chí "Done khi" đạt — `TelegramAlerter` với retry 3 lần (tenacity NetworkError), dedup 5 phút (AlertDeduper), rate limit max 3 alerts/giờ/pipeline, token masking trong log và terminal. `_heartbeat_watchdog_tick` trong daemon cảnh báo scheduler dead khi last_beat > 2h. `format_sync_fail_alert()` + `format_scheduler_dead_alert()` escape Markdown chars (backtick/asterisk/underscore). CLI `flowbyte alert test` hoạt động. 42/42 unit tests xanh (test_telegram_alerter.py + test_us005_alerting.py).
> **✅ Done (US-006) — 2026-04-25:** 4 validation rules (fetch_upsert_parity, volume_sanity, weekly_full_drift, soft_delete_sanity) persist vào `validation_results`, auto-run sau successful sync. ValidationExecutor load prev_runs với sync_id filter (tránh circular reference). 13 E2E integration tests (testcontainers Postgres) cover full acceptance criteria. Security fix: EventName.VALIDATION_FAILED (không phải SYNC_FAILED) khi executor crash — tránh false Telegram "sync failed" alert. 58 unit tests xanh (test_us006_validation.py).
> **⚠️ Partial (US-008):** code structure, chưa implement đầy đủ — được plan cho Sprint 3.
...
> **🔒 Security Review (US-004) — 2026-04-24:** 3 lỗ hổng đã được phát hiện và fix: (1) MEDIUM — `sync.py run` thiếu validation cho `pipeline` / `resource` / `mode` trước khi insert DB → fixed bằng `_assert_valid_name()` + resource whitelist `PHASE_1_RESOURCES` + mode enum check; (2) LOW — SSRF: shop domain regex `[a-zA-Z0-9\-\.]` cho phép IP address (e.g. `192.168.1.1`) → fixed bằng regex mới `*.myharavan.com` only; (3) LOW — Telegram Markdown injection: backtick trong error string phá vỡ code block format → fixed bằng `error.replace("\`", "'")`. 15 security tests mới (test_cli_sync.py + TestCredsSetShopDomainValidation), tổng 401 unit tests xanh. `/security-review` full scan: 4 candidate khác verify là false positive (hardcoded default = pre-existing + env-override; host injection = requires system access đã có; error display = chỉ operator thấy; `_raw` full record = thiết kế có chủ đích per PRD §⑦). **Post-audit (2026-04-24):** Thêm fix LOW — Telegram Markdown `*` + `_` injection trong `format_sync_fail_alert()` → `safe_error` escape thêm `*→""` và `_→" "`.
> **🔒 Full Audit (US-001→US-004) — 2026-04-24:** Toàn bộ US-001→US-004 được review lại lần cuối trước Sprint 2. Security verified: path traversal (SAFE), AES-256-GCM (CORRECT), master key chmod 600 (ENFORCED), SQL injection via SQLAlchemy (SAFE), SSRF shop domain `*.myharavan.com` (PROTECTED), token bucket thread-safe (VERIFIED), checkpoint atomicity (CORRECT). Tổng 2 bugs fix + 401/401 tests xanh. Sprint 2 bắt đầu với **US-007** (CLI Observability).
> **✅ Done (US-007) — 2026-04-25:** Tất cả tiêu chí "Done khi" đạt — 5 CLI commands (`status` / `history` / `logs` / `inspect`) hoạt động đầy đủ. 4 trạng thái 🟢 OK / 🔴 FAIL / 🟡 RUNNING / ⚪ DISABLED hiển thị đúng. `status` load < 1s (performance test). `history` đủ columns (time/resource/mode/status/fetched/duration/error). `logs --errors` filter chính xác ERROR/CRITICAL. `inspect` deep-trace timeline + validation results. Scheduler footer (🟢 Running uptime / ❌ Dead). Validation summary footer (✅/⚠️/❌). 48 unit tests xanh (36 functional + 12 security: `TestInputValidationSecurity`). 41 E2E integration tests viết xong (`test_us007_observability_e2e.py`). **🔒 Security Review (US-007) — 2026-04-25:** 4 lỗ hổng phát hiện và fix: (1) HIGH — LIKE wildcard injection trong `inspect` — `sync_id` user input không validated → `%` match bất kỳ row → fixed bằng `_assert_valid_sync_id()` regex `^[a-f0-9-]{1,36}$`; (2) MEDIUM — Rich markup injection trong `logs` — `r.event` + `r.message` từ DB không escaped → fixed bằng `escape()` từ `rich.markup`; (3) MEDIUM — Rich markup injection trong `inspect` timeline — `log_entry.event`, `log_entry.message`, `v.rule` không escaped → fixed; (4) LOW — `error_short` trong Rich Table cell không escaped → fixed; (5) LOW — `--last` không có upper bound → fixed `min=1, max=100`. `/security-review` full scan: 4 candidate khác verify false positive (run.status = DB internal only; OverflowError = DoS trusted operator; missing CheckConstraint = hardening gap; v.status = DB CHECK constraint bảo vệ). 48/48 unit tests xanh.
...
|---|---|---|---|
| 2026-04-25 | **v16** | **US-006 → ✅ Done:** 4 validation rules (fetch_upsert_parity/volume_sanity/weekly_full_drift/soft_delete_sanity), ValidationExecutor, auto-run sau successful sync. 13 E2E integration tests (testcontainers Postgres). Security fix: EventName.SYNC_FAILED → VALIDATION_FAILED khi executor crash (tránh false Telegram alert); sync_id filter trong `_load_prev_runs()`. 58/58 unit tests xanh. | Duy + Claude |
| 2026-04-25 | **v15** | **US-007 → ✅ Done:** 5 CLI commands (status/history/logs/inspect), 4 status types, performance <1s, scheduler footer, validation footer. 48 unit tests xanh (36 functional + 12 security). 41 E2E integration tests. Security review: 5 lỗ hổng fix (LIKE wildcard injection, Rich markup injection ×3, --last unbounded). `/security-review` 4 candidate verify false positive. | Duy + Claude |
| 2026-04-24 | **v14** | **US-005 → ✅ Done:** TelegramAlerter (retry/dedup/rate-limit/masking), _heartbeat_watchdog_tick (scheduler dead >2h), format functions (Markdown escape), CLI `alert test`. 42 unit tests xanh. PRD status table + Done note cập nhật. | Duy + Claude |
| 2026-04-24 | **v13** | **Full audit US-001→US-004 trước Sprint 2:** Fix 2 bugs — (1) MEDIUM: `runner.py` variants transform hardcode `None` → pass `variant_resource_cfg.transform` từ config; (2) LOW: `telegram.py` Markdown injection `*`/`_` chưa escaped → thêm `.replace("*","").replace("_"," ")`. US-004 table status fix → ✅ Done. Known limitations documented: variants soft-delete by design, inventory_levels silent if enabled. Security audit: path traversal/AES/SSRF/SQL/token-bucket tất cả SAFE. Sprint 2 bắt đầu với US-007. 401/401 unit tests xanh. | Duy + Claude |
| 2026-04-23 | v1 | Draft đầu tiên | Duy |
| 2026-04-24 | **v12** | **E2E hoàn thành tất cả resources:** US-004 → ✅ Done. 4 bugs E2E mới: (3) `inventory_levels` Haravan trả 200+empty body (không phải 404) — không có endpoint; inventory data ở `variants.inventory_advance` JSONB → resource disabled, schema variants cập nhật thêm column `inventory_advance JSONB`; (4) Full refresh mode vẫn pass checkpoint vào paginate → records bị skip toàn bộ → fix `paginate_checkpoint=None` khi mode=full_refresh trong tất cả extractors (orders/customers/products). Kết quả: 5 resources xanh, inventory_advance populated. | Duy + Claude |
